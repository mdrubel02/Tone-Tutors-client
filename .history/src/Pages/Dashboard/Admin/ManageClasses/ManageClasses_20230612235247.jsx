...
const ManageClasses = () => {
    ...
    const onSubmit = async (data) => {
        const { status } = data;
        console.log(status,updateModal);
    }
    return (
                            ...
                            manageClasses.map((manageClass, index) => <tr key={manageClass._id}>
                                ...
                                <td>
                                    <div className='flex justify-center'>
                                        <button className='me-3 bg-primary btn btn-sm text-white'>Approved</button>
                                        <a onClick={() => setUpdateModal(manageClass?._id)} className="cursor-pointer inline-flex items-center justify-center w-20 py-1 px-1 font-semibold tracking-wide text-white transition duration-200 rounded-full shadow-md outline-none bg-primary btn btn-sm" href="#my-modal-2" >Denied</a>
                                    </div>
                                </td>
                            </tr>)
                        }
